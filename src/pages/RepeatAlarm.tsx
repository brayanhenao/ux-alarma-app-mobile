// create a create alarm component
import * as React from 'react';
import { View } from 'react-native';

import { Button, Checkbox, List, Text, Divider } from 'react-native-paper';
import Context from '../Context';

const RepeatAlarm = ({ navigation }) => {
    ...
    const addRepeatToAlarm = () => {
        const repeat = daysOfWeekChecked.map((day, index) => {
            if (day) {
    ...
    const setDayOfWeekChecked = (index:number) => {
        setDaysOfWeekChecked(daysOfWeekChecked.map((day, i) => {
            if (i === index) {
                return !day
            }
            return day
                            ...
                            daysOfWeek.map((day, index) => {
                                return (
                                    <View style={{alignItems:"center"}} key={index}>
                                        <Checkbox status={daysOfWeekChecked[index]?'checked':'unchecked'} onPress={()=>setDayOfWeekChecked(index)} />
