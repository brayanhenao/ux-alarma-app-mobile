...
const Picker: React.FC<{
  ...
  initialValue: string;
}> = ({ elements = [], onChange = () => {}, initialValue = '' }) => {
  return (
    <View
      style={{
        alignItems: 'center',
...
const TimePicker: React.FC<{ initialValue?: string; onChange: (val: string) => void }> = ({
  initialValue = '12:01 AM',
  onChange = () => {},
}) => {
  ...
  return (
    <View style={{ display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
      <Picker
        elements={Array.from({ length: 12 }, (_, i) => `${i + 1}`.padStart(2, '0'))}
        onChange={setHour}
        initialValue={hour}
      />
      <Text>:</Text>
      ...
      <Picker
        elements={Array.from({ length: 60 }, (_, i) => `${i}`.padStart(2, '0'))}
        onChange={setMinute}
        initialValue={minute}
      />
